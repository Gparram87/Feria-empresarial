...
public class Parqueadero {
    ...
    public String metodo1()
    {
        ...
        int TiempoMaximo = 0;
        String placaAuto = null;
        for (Puesto posicion : puestos)
        {
            if (posicion.estaOcupado())
            {
                Carro UbicarAuto = puestos[posicion.darNumeroPuesto()].darCarro();
                if (UbicarAuto.darTiempoEnParqueadero(horaActual) > TiempoMaximo)
                {
                    TiempoMaximo= UbicarAuto.darTiempoEnParqueadero(horaActual);
                    placaAuto = UbicarAuto.darPlaca();
                }
            }
        }
        return "El auto que llava mas tiempo " + placaAuto;
    }
    ...
    public String metodo2()
    {
        ...
        for (Puesto p : puestos)
        {
            if (p.estaOcupado())
            {
                CarroParqueado=CarroParqueado+1;
            }
        }
        porcentaje = (float) CarroParqueado / tot_puesto;
